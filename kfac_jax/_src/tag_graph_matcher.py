...
class JaxprGraph:
  ...
  Attributes:
    ...
    manual_registrations: Any layer tag equations that have been manually
      registered.
    jaxpr: The underlying :class:`jex.core.Jaxpr` part of ``self.closed_jaxpr``.
    consts: The underlying constants part ``self.closed_jaxpr``.
    outvars: The output variables of the underlying :class:`jex.core.Jaxpr` part
...
class GraphPattern:
  ...
  Attributes:
    ...
    in_values_preprocessor: A function that can optionally modify the in_vals
      ...
      the jaxpr.
    jaxpr: The underlying :class:`jex.core.Jaxpr` represented by the pattern.
    param_vars: The list of :class:`jex.core.Var` that correspond to parameters
      in the pattern.
...
def match_equations(
  ...
  def add_vars_if_possible(
    ...
    least one of the graph variables is a :class:`~jex.core.Literal` (meaning a
