...
class BlockDiagonalCurvature(
  ...
  """Block diagonal curvature estimator class.
    ...
    Supports for the following estimation modes:
      ...
      * fisher_empirical_direct - similar to fisher_empirical, but bypasses the
        ...
        statistics by applying jax.grad to the value function. This will
        typically be faster than fisher_empirical (as the gradient computation
        will be merged by XLA with the one done for the optimizer). Currently,
        this mode only supports curvature block approximations that use
        parameter gradient information.
  ...
  def __init__(
    ...
    Args:
      func: The loss function, which should have at least one registered loss.
        For the purposes of the ``'fisher_empirical_direct_[synced]'``
        estimation modes, this function should be normalized by the batch size.
        (For other estimation modes it won't matter either way.)
  ...
  def update_curvature_matrix_estimate(
    ...
    if estimation_mode == "fisher_gradients":
      ...
      # the optimizer. Foruntately, XLA should optimize it away.
